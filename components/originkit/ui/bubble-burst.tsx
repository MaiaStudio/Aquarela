 ...
 */
function makeEaseFn(transition: Record<string, unknown> | null | undefined): {
    duration: number
    delay: number
    ...
    const raw =
        Array.isArray(transition?.ease) && transition.ease.length === 4
            ? transition.ease
            : [0.4, 0, 0.5, 1]
    const [x1, y1, x2, y2] = raw.map((n: unknown) =>
...
export default function BubbleBurst(props: BubbleBurstProps) {
    ...
    const cfg = React.useMemo<Config>(() => ({
        tint,
        sheen,
        iridescence,
        rim,
    ...
    useEffect(() => {
        ...
        try {
            scene = new BubbleScene(container, cfg)
        } catch {
